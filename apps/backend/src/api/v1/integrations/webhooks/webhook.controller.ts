...
export const handleCompanyWebhook = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        await requestContext.run({ companyId }, async () => {
            // 1. Verify Signature
            try {
                const providerAdapter = await CompanyPaymentService.getActiveProvider(companyId);
            ...
            if (providerName === 'razorpay') {
                ...
                if (eventType === 'payment.captured' || eventType === 'payment.authorized' || eventType === 'payment_link.paid') {
                    ...
                    await BankVerificationService.handleValidationWebhook(req.body);
                }
