    ...
    try {
        ...
        if (req.file) {
            try {
                ...
                console.error('[Register Tenant] Logo upload failed:', err.message);
            }
        }
...
exports.registerTenantGoogle = async (req, res) => {
    try {
        ...
        if (req.file) {
            try {
                ...
                console.error('[Register Tenant Google] Logo upload failed:', err.message);
            }
        }
        ...
        console.error('Register Tenant Google Error:', err);
        res.status(500).json({ success: false, error: 'Failed to register company.' });
    }
};
...
exports.configureTenant = async (req, res) => {
    try {
        ...
        // Create mapping
        await prisma.tenantUserMapping.upsert({
            where: {
                email_companyId: {
        ...
        res.status(500).json({ success: false, error: err.message || 'Failed to configure tenant.' });
    }
};
