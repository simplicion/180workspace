...
class TimelineOps {
  ...
  static EditIrCanvas canvasFor(String aspect, {num fps = 30, String background = '#000000'}) => switch (aspect) {
        '16:9' => EditIrCanvas(aspect: aspect, width: 1920, height: 1080, fps: fps, background: background),
        '1:1' => EditIrCanvas(aspect: aspect, width: 1080, height: 1080, fps: fps, background: background),
        '4:5' => EditIrCanvas(aspect: aspect, width: 1080, height: 1350, fps: fps, background: background),
        _ => EditIrCanvas(aspect: '9:16', width: 1080, height: 1920, fps: fps, background: background),
      };

  /// Centre crop of a [srcW]×[srcH] (display-oriented) source that fills [canvas]; null when the
  /// aspect ratios already match (no crop needed).
  static EditIrCrop? centerCrop(int srcW, int srcH, EditIrCanvas canvas) {
    ...
    if (src > dst) {
      final w = dst / src;
      return EditIrCrop(x: _r((1 - w) / 2), y: 0, width: _r(w), height: 1);
    }
    final h = src / dst;
    return EditIrCrop(x: 0, y: _r((1 - h) / 2), width: 1, height: _r(h));
  }
  ...
  static MobileEditIr initial({
    ...
    if (durationMs < minClipMs) {
      throw MediaEngineException('INVALID_SOURCE', 'The video is too short to edit (${durationMs}ms).');
    }
    ...
    final clip = EditIrClip(
      ...
      crop: centerCrop(width, height, canvas),
    );
    final ir = MobileEditIr(
      projectId: projectId,
      canvas: canvas,
      durationMs: durationMs,
      sources: [EditIrSource(durationMs: durationMs, width: width, height: height)],
      clips: [clip],
    );
  ...
  static int? sourceToTimeline(MobileEditIr ir, int sourceMs, {String assetId = 'primary'}) {
    for (final c in ir.clips) {
      if (c.assetId == assetId && sourceMs >= c.sourceStartMs && sourceMs < c.sourceEndMs) {
        return c.timelineStartMs + ((sourceMs - c.sourceStartMs) / c.speed).round();
      }
    }
    return null;
  }
  ...
  static List<List<int>> speechRanges(List<TranscriptWord> words, MobileEditIr ir) {
    final out = <List<int>>[];
  ...
  static MobileEditIr _replaceClips(MobileEditIr ir, List<EditIrClip> newClips) {
    if (newClips.isEmpty) throw MediaEngineException('INVALID_EDIT', 'A video needs at least one clip.');
    final laid = _relayout(newClips);
    final segs = <_Seg>[];
    for (final n in laid) {
      final old = ir.clips.where((o) =>
          o.assetId == n.assetId && n.sourceStartMs >= o.sourceStartMs && n.sourceEndMs <= o.sourceEndMs).firstOrNull;
      if (old == null) continue; // newly revealed source (trim extend): nothing to carry over
      segs.add(_Seg(
        old.timelineStartMs + (n.sourceStartMs - old.sourceStartMs) / old.speed,
    ...
    for (final c in ir.captions) {
      ...
      captions.add(EditIrCaption(
        id: c.id,
        kind: c.kind,
        startMs: r.$1,
        endMs: r.$2,
        text: c.kind == 'caption' && words.isNotEmpty ? words.map((w) => w.text).join(' ') : c.text,
        words: c.kind == 'text' ? [EditIrWord(text: c.text, startMs: r.$1, endMs: r.$2)] : words,
        style: c.style,
      ));
    }
    ...
    final music = <EditIrMusic?>[
      for (final m in ir.audio.music)
        () {
          ...
          final full = m.timelineStartMs == 0 && m.timelineEndMs >= ir.durationMs - 1;
          final r = full ? (0, duration) : map.range(m.timelineStartMs, m.timelineEndMs);
    ...
    final out = MobileEditIr(
      ...
      zooms: zooms,
      audio: EditIrAudio(originalVolumeDb: ir.audio.originalVolumeDb, music: music, speechRangesMs: mergedSpeech),
    );
    out.validate();
    return out;
  }

  static EditIrMusic _music(EditIrMusic m,
          {int? start, int? end, double? volumeDb, int? fadeInMs, int? fadeOutMs, EditIrDuck? duck, bool clearDuck = false}) =>
      EditIrMusic(
        id: m.id,
  ...
  static MobileEditIr _withAudio(MobileEditIr ir, {double? originalVolumeDb, List<EditIrMusic>? music, List<List<int>>? speechRangesMs}) =>
      _copy(ir,
          audio: EditIrAudio(
  ...
  static EditIrClip _clip(EditIrClip c,
          {String? id,
          int? sourceStartMs,
          int? sourceEndMs,
          double? speed,
  ...
  static int clipIndexAt(MobileEditIr ir, int timelineMs) {
    for (var i = 0; i < ir.clips.length; i++) {
      if (timelineMs >= ir.clips[i].timelineStartMs && timelineMs < ir.clips[i].timelineEndMs) return i;
    }
    return ir.clips.length - 1;
  }

  static Iterable<int> _targets(MobileEditIr ir, int? index) =>
      index == null ? Iterable.generate(ir.clips.length) : [index.clamp(0, ir.clips.length - 1)];
  ...
  static MobileEditIr split(MobileEditIr ir, int timelineMs) {
    ...
    final c = ir.clips[i];
    final at = c.sourceStartMs + ((timelineMs - c.timelineStartMs) * c.speed).round();
    if (at - c.sourceStartMs < minClipMs || c.sourceEndMs - at < minClipMs) {
      throw MediaEngineException('INVALID_EDIT', 'Move the playhead away from the clip edge to split.');
    }
  ...
  static MobileEditIr deleteClip(MobileEditIr ir, int index) {
    if (ir.clips.length == 1) throw MediaEngineException('INVALID_EDIT', 'This is the only clip. Trim it instead.');
  ...
  static MobileEditIr removeRange(MobileEditIr ir, int startMs, int endMs) {
    ...
    for (final c in ir.clips) {
      ...
      final cutS = c.sourceStartMs + ((math.max(startMs, c.timelineStartMs) - c.timelineStartMs) * c.speed).round();
      final cutE = c.sourceStartMs + ((math.min(endMs, c.timelineEndMs) - c.timelineStartMs) * c.speed).round();
      if (cutS - c.sourceStartMs >= minClipMs) clips.add(_clip(c, sourceEndMs: cutS));
      if (c.sourceEndMs - cutE >= minClipMs) {
        clips.add(_clip(c, id: cutS - c.sourceStartMs >= minClipMs ? _id('c') : c.id, sourceStartMs: cutE));
      }
    }
    if (clips.isEmpty) throw MediaEngineException('INVALID_EDIT', 'That would remove the whole video.');
    return _replaceClips(ir, clips);
  }

  /// Sets the source in/out of clip [index] (bounded by the source length).
  static MobileEditIr trim(MobileEditIr ir, int index, {required int sourceStartMs, required int sourceEndMs}) {
    final c = ir.clips[index];
    final srcLen = ir.sources.where((s) => s.assetId == c.assetId).firstOrNull?.durationMs ?? c.sourceEndMs;
    final s = sourceStartMs.clamp(0, srcLen);
    final e = sourceEndMs.clamp(0, srcLen);
    if (e - s < minClipMs) throw MediaEngineException('INVALID_EDIT', 'A clip must be at least ${minClipMs}ms long.');
    return _replaceClips(ir, [...ir.clips]..[index] = _clip(c, sourceStartMs: s, sourceEndMs: e));
  }
  ...
  static MobileEditIr setSpeed(MobileEditIr ir, double speed, {int? index}) {
    ...
    return _replaceClips(ir, [for (var i = 0; i < ir.clips.length; i++) targets.contains(i) ? _clip(ir.clips[i], speed: s) : ir.clips[i]]);
  }
  ...
  static MobileEditIr setClipVolume(MobileEditIr ir, double db, {int? index}) {
    final targets = _targets(ir, index).toSet();
    return _copy(ir, clips: [for (var i = 0; i < ir.clips.length; i++) targets.contains(i) ? _clip(ir.clips[i], volumeDb: db.clamp(-60.0, 12.0)) : ir.clips[i]]);
  }

  static MobileEditIr setFilter(MobileEditIr ir, EditIrFilter? filter, {int? index}) {
    final targets = _targets(ir, index).toSet();
    return _copy(ir, clips: [
      for (var i = 0; i < ir.clips.length; i++)
        targets.contains(i) ? _clip(ir.clips[i], filter: filter, clearFilter: filter == null) : ir.clips[i],
    ]);
  }

  /// Transition into clip [index] (≥ 1), or at every cut when null.
  static MobileEditIr setTransition(MobileEditIr ir, EditIrTransition? t, {int? index}) {
    final targets = _targets(ir, index).where((i) => i > 0).toSet();
    return _copy(ir, clips: [
      for (var i = 0; i < ir.clips.length; i++)
        targets.contains(i) ? _clip(ir.clips[i], transitionIn: t, clearTransition: t == null) : ir.clips[i],
    ]);
  }
  ...
  static MobileEditIr rotate(MobileEditIr ir, {int? index, bool rotate90 = true, bool toggleFlip = false}) {
    final targets = _targets(ir, index).toSet();
    return _copy(ir, clips: [
      for (var i = 0; i < ir.clips.length; i++)
        if (!targets.contains(i))
          ...
          () {
            ...
            final src = ir.sources.where((s) => s.assetId == c.assetId).firstOrNull;
            final sideways = deg == 90 || deg == 270;
            final crop = src == null ? c.crop : centerCrop(sideways ? src.height : src.width, sideways ? src.width : src.height, ir.canvas);
            return _clip(c, rotationDeg: deg, flipH: toggleFlip ? !c.flipH : c.flipH, crop: crop, clearCrop: crop == null);
          }(),
    ]);
  }
  ...
  static MobileEditIr setCrop(MobileEditIr ir, EditIrCrop? crop, {int? index}) {
    if (crop != null && (crop.x < 0 || crop.y < 0 || crop.width <= 0 || crop.height <= 0 || crop.x + crop.width > 1.0001 || crop.y + crop.height > 1.0001)) {
      throw MediaEngineException('INVALID_EDIT', 'The crop must stay inside the frame.');
    }
    final targets = _targets(ir, index).toSet();
    return _copy(ir, clips: [
      for (var i = 0; i < ir.clips.length; i++) targets.contains(i) ? _clip(ir.clips[i], crop: crop, clearCrop: crop == null) : ir.clips[i],
    ]);
  }

  /// Changes the output aspect. With [fill] every clip is centre-cropped to fill the frame;
  /// otherwise the source is letterboxed on [background].
  static MobileEditIr setAspect(MobileEditIr ir, String aspect, {bool fill = true, String? background}) {
    final canvas = canvasFor(aspect, fps: ir.canvas.fps, background: background ?? ir.canvas.background);
    return _copy(ir, canvas: canvas, clips: [
      for (final c in ir.clips)
        () {
          final src = ir.sources.where((s) => s.assetId == c.assetId).firstOrNull;
          final crop = fill && src != null ? centerCrop(src.width, src.height, canvas) : null;
  ...
  static Map<String, dynamic> captionStyle(String preset, {String highlightColor = '#FFE600', double positionY = 0.72}) {
    ...
    return switch (preset) {
      'BOLD_POP' => {...base, 'animation': 'word_pop', 'fontSizePx': 72, 'uppercase': true},
      'KARAOKE' => {...base, 'animation': 'karaoke'},
      'BOXED' => {...base, 'strokeWidthPx': 0, 'shadow': false, 'fontWeight': 600, 'background': {'color': '#000000B3', 'paddingPx': 16, 'radiusPx': 16}},
      'TITLE' => {...base, 'fontSizePx': 88, 'positionY': positionY},
      _ => {...base, 'preset': 'CLEAN', 'fontWeight': 600, 'strokeWidthPx': 4},
    };
  }

  static const captionPresets = {'CLEAN': 'Clean', 'BOLD_POP': 'Bold pop', 'KARAOKE': 'Karaoke', 'BOXED': 'Boxed'};
  ...
  static MobileEditIr autoCaptions(MobileEditIr ir, List<TranscriptWord> words,
      {String preset = 'BOLD_POP', int wordsPerCaption = 3, String highlightColor = '#FFE600', double positionY = 0.72}) {
    if (words.isEmpty) throw MediaEngineException('NO_TRANSCRIPT', 'Captions need a transcript. Transcribe the video first.');
    final style = captionStyle(preset, highlightColor: highlightColor, positionY: positionY);
    final mapped = <EditIrWord>[];
    for (final w in words) {
      final s = sourceToTimeline(ir, w.startMs);
      if (s == null) continue;
      final e = sourceToTimeline(ir, math.max(w.startMs, w.endMs - 1)) ?? s;
      mapped.add(EditIrWord(text: w.text, startMs: s, endMs: math.max(e + 1, s + 1)));
    }
    final captions = <EditIrCaption>[];
    ...
    for (var i = 0; i < captions.length - 1; i++) {
      ...
      if (a.endMs > b.startMs) {
        captions[i] = EditIrCaption(
            ...
            endMs: b.startMs,
            text: a.text,
            words: [for (final w in a.words) EditIrWord(text: w.text, startMs: w.startMs, endMs: math.min(w.endMs, b.startMs))],
            style: a.style);
      }
    }
    return _copy(ir, captions: [...ir.captions.where((c) => c.kind == 'text'), ...captions]..sort((a, b) => a.startMs.compareTo(b.startMs)));
  }

  /// Restyles every speech caption.
  static MobileEditIr styleCaptions(MobileEditIr ir, String preset, {String highlightColor = '#FFE600', double? positionY}) {
    final style = captionStyle(preset, highlightColor: highlightColor, positionY: positionY ?? 0.72);
    return _copy(ir, captions: [
      for (final c in ir.captions)
        c.kind == 'caption' ? EditIrCaption(id: c.id, startMs: c.startMs, endMs: c.endMs, text: c.text, words: c.words, style: style) : c,
    ]);
  }

  /// A title / lower third from [startMs] for [durationMs].
  static MobileEditIr addText(MobileEditIr ir, String text, {required int startMs, required int durationMs, double positionY = 0.2}) {
    final t = text.trim();
    if (t.isEmpty) throw MediaEngineException('INVALID_EDIT', 'Type some text first.');
    ...
    return _copy(ir, captions: [...ir.captions, cap]..sort((a, b) => a.startMs.compareTo(b.startMs)));
  }

  static MobileEditIr removeCaption(MobileEditIr ir, String id) => _copy(ir, captions: ir.captions.where((c) => c.id != id).toList());

  static MobileEditIr clearCaptions(MobileEditIr ir) => _copy(ir, captions: ir.captions.where((c) => c.kind == 'text').toList());

  // ── Camera ─────────────────────────────────────────────────────────────────

  /// Punch-in zoom; rejected if it would overlap another zoom.
  static MobileEditIr addZoom(MobileEditIr ir, {required int startMs, int durationMs = 1500, double scale = 1.3, double centerX = 0.5, double centerY = 0.45}) {
    final s = startMs.clamp(0, math.max(0, ir.durationMs - 300)).toInt();
    final e = math.min(ir.durationMs, s + math.max(durationMs, 300)).toInt();
    if (ir.zooms.any((z) => s < z.endMs && e > z.startMs)) {
      throw MediaEngineException('INVALID_EDIT', 'There is already a zoom here.');
    }
    final z = EditIrZoom(id: _id('z'), startMs: s, endMs: e, scale: scale.clamp(1.0, 4.0), centerX: centerX, centerY: centerY, rampMs: math.min(250, (e - s) ~/ 2));
    return _copy(ir, zooms: [...ir.zooms, z]..sort((a, b) => a.startMs.compareTo(b.startMs)));
  }

  static MobileEditIr removeZoom(MobileEditIr ir, String id) => _copy(ir, zooms: ir.zooms.where((z) => z.id != id).toList());

  // ── B-roll ─────────────────────────────────────────────────────────────────

  /// B-roll over [startMs, startMs+durationMs). [source] is `{kind:url,url}` or `{kind:asset,assetId}`.
  static MobileEditIr addBroll(MobileEditIr ir, Map<String, dynamic> source, {required int startMs, int durationMs = 3000}) {
    final s = startMs.clamp(0, math.max(0, ir.durationMs - 300)).toInt();
    final e = math.min(ir.durationMs, s + math.max(durationMs, 300)).toInt();
    final o = EditIrOverlay(id: _id('b'), timelineStartMs: s, timelineEndMs: e, source: source);
    return _copy(ir, overlays: [...ir.overlays, o]..sort((a, b) => a.timelineStartMs.compareTo(b.timelineStartMs)));
  }

  static MobileEditIr removeOverlay(MobileEditIr ir, String id) => _copy(ir, overlays: ir.overlays.where((o) => o.id != id).toList());

  // ── Audio ──────────────────────────────────────────────────────────────────

  static MobileEditIr setOriginalVolume(MobileEditIr ir, double db) => _withAudio(ir, originalVolumeDb: db.clamp(-60.0, 12.0));

  /// Background music under the whole video from an HTTPS [url] (upload local files first).
  static MobileEditIr setMusic(MobileEditIr ir,
      {required String url, String? title, double volumeDb = -16, int fadeInMs = 500, int fadeOutMs = 1000, bool duckUnderSpeech = true}) {
    final uri = Uri.tryParse(url);
    if (uri == null || !uri.isScheme('https')) throw MediaEngineException('INVALID_EDIT', 'Music must be an https URL.');
    final m = EditIrMusic(
      ...
      fadeInMs: fadeInMs,
      fadeOutMs: fadeOutMs,
      duck: EditIrDuck(enabled: duckUnderSpeech && ir.audio.speechRangesMs.isNotEmpty),
    );
    return _withAudio(ir, music: [m]);
  }

  static MobileEditIr updateMusic(MobileEditIr ir, {double? volumeDb, int? fadeInMs, int? fadeOutMs, bool? duck}) {
    ...
    return _withAudio(ir, music: [
      _music(m,
          ...
          fadeInMs: fadeInMs,
          fadeOutMs: fadeOutMs,
          duck: duck == null ? null : EditIrDuck(enabled: duck && ir.audio.speechRangesMs.isNotEmpty)),
    ]);
  }

  static MobileEditIr removeMusic(MobileEditIr ir) => _withAudio(ir, music: const []);
}
...
class _Mapper {
  ...
  (int, int)? range(int a, int b) {
    ...
    for (final s in segs) {
      ...
      if (first == null || math.max(s.oldStart, a.toDouble()) < math.max(first.oldStart, a.toDouble())) first = s;
    }
    _Seg? last;
    for (final s in segs) {
      if (s.oldEnd <= a || s.oldStart >= b) continue;
      if (last == null || math.min(s.oldEnd, b.toDouble()) > math.min(last.oldEnd, b.toDouble())) last = s;
    }
