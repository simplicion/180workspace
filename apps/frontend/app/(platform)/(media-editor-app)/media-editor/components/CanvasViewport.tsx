...
export const CanvasViewport: React.FC<CanvasViewportProps> = ({
  ...
  // Determine active clips and assets across all video tracks at current playhead
  const sortedVideoTracks = [...(editIR.tracks.videoTracks || [])].sort(
    (a, b) => (a.zIndex ?? 0) - (b.zIndex ?? 0)
  );

  const activeTrackLayers = sortedVideoTracks.map((track) => {
    ...
    const matchedAsset = assets.find((a) => a.id === activeClip.assetId) || {
      id: activeClip.assetId,
      name: activeClip.sourcePath.split(/[\/\\]/).pop() || "media",
      filePath: activeClip.sourcePath,
      mimeType: activeClip.sourcePath.match(/\.(jpg|jpeg|png|webp|gif|svg)($|\?)/i) ? "image/jpeg" : "video/mp4",
    };

    const isImg = Boolean(
      matchedAsset?.mimeType?.startsWith("image/") ||
      matchedAsset?.filePath?.match(/\.(jpg|jpeg|png|webp|gif|svg)($|\?)/i) ||
      activeClip?.sourcePath?.match(/\.(jpg|jpeg|png|webp|gif|svg)($|\?)/i)
    );
    ...
    track: (typeof editIR.tracks.videoTracks)[0];
    clip: (typeof editIR.tracks.videoTracks)[0]["clips"][0];
    asset: any;
    isImage: boolean;
    src: string;
  }>;

  // Main track active clip (bottom layer)
  const mainLayer = activeTrackLayers.find((l) => l.track.type === "MAIN_VIDEO") || activeTrackLayers[0] || null;
  const overlayLayers = activeTrackLayers.filter((l) => l !== mainLayer);

  const activeVideoClip = mainLayer?.clip || null;
  const activeAsset = mainLayer?.asset || null;
  const activeVideoSrc = mainLayer?.src || null;
  const isImage = mainLayer?.isImage ?? false;

  // Active clip spatial transforms & crop for main layer
          ...
          {hasActiveMedia ? (
            ...
            >
              <div className="w-full h-full relative flex items-center justify-center">
                ...
                {/* Composited Secondary / Overlay Layers (Stickers, Images, PiP, B-Roll) */}
                {overlayLayers.map((layer) => {
                  const oScale = layer.clip.transform?.scale?.start ?? 1.0;
                  const oPosX = layer.clip.transform?.position?.x ?? 0;
                  ...
                  return (
                    <div
                      key={layer.clip.id}
                      className="absolute max-w-[40%] max-h-[40%] flex items-center justify-center pointer-events-none select-none transition-all duration-100 ease-out z-10"
                      style={{
                        transform: `translate(${oPosX}px, ${oPosY}px) scale(${oScale}) rotate(${oRot}deg)`,
                        opacity: oOpacity,
                      }}
                    >
                      {layer.isImage ? (
                      ...
                      {isSelected && (
                        <div className="absolute inset-0 border-2 border-emerald-400/90 rounded-lg pointer-events-none" />
                      )}
                    </div>
                  );
                })}
