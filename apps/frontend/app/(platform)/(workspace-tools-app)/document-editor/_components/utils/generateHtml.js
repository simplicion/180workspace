...
exports.generateHtmlFromBlocks = generateHtmlFromBlocks;
const headerFooterTemplates_1 = require("../../../_components/headerFooterTemplates");
const autoLinkUrls_1 = require("./autoLinkUrls");
function generateHtmlFromBlocks(blocks, documentDetails, designSettings, brandConfig, fragmentOnly = false) {
    ...
    const replaceVars = (text) => {
        ...
        return (0, autoLinkUrls_1.autoLinkUrls)(replaced);
    };
    let finalBlocks = [...blocks];
    if (designSettings?.selectedHeaderId && designSettings.selectedHeaderId !== 'none') {
        const template = headerFooterTemplates_1.HEADER_TEMPLATES.find(t => t.id === designSettings.selectedHeaderId);
        if (template) {
            finalBlocks = [...template.blocks, ...finalBlocks];
        }
    }
    if (designSettings?.selectedFooterId && designSettings.selectedFooterId !== 'none') {
        const template = headerFooterTemplates_1.FOOTER_TEMPLATES.find(t => t.id === designSettings.selectedFooterId);
        if (template) {
    ...
    const blocksHtml = finalBlocks.map(block => {
        if (block.visibilityRule && typeof block.visibilityRule === 'object' && block.visibilityRule.field && block.visibilityRule.operator && block.visibilityRule.value !== undefined) {
            ...
            switch (block.visibilityRule.operator) {
                case 'equals':
                    isVisible = fieldValue === ruleValue;
                    break;
                case 'not_equals':
                    isVisible = fieldValue !== ruleValue;
                    break;
                case 'contains':
                    isVisible = String(fieldValue || '').includes(String(ruleValue));
                    break;
                case 'not_contains':
                    isVisible = !String(fieldValue || '').includes(String(ruleValue));
                    break;
                case 'greater_than':
                    isVisible = Number(fieldValue) > Number(ruleValue);
                    break;
                case 'less_than':
                    isVisible = Number(fieldValue) < Number(ruleValue);
                    break;
        ...
        switch (block.type) {
            ...
            case 'text': {
                ...
                if (!textContent.trim())
                    return ''; // don't render empty text blocks (e.g. if GST is empty)
            ...
            `;
                        ...
                        if (url)
                            childInnerHtml = `<img src="${url}" alt="Container Image" style="max-width: 100%; height: auto; border-radius: 4px;" />`;
                    }
          ...
          ${childrenHtml}
            ...
            case 'image': {
                ...
                if (!url || url.trim() === '')
                    return ''; // don't render image if url is empty
            ...
            <div style="background-color: #F8FAFC; padding: 14px 18px; border-top: 1px solid #E2E8F0; display: flex; justify-content: flex-end;">
              <div style="width: 240px; font-size: ${baseSize - 2}px; line-height: 1.8;">
                ...
                const colWidths = block.content?.colWidths;
