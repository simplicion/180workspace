...
export default function EditRuleModal({ isOpen, onClose, linkId, rule, onSuccess }: EditRuleModalProps) {
  ...
  useEffect(() => {
    if (rule) {
      ...
      setWeight(rule.weight ?? 100);
      setConditions(Array.isArray(rule.conditions) && rule.conditions.length > 0 ? rule.conditions : [
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const res = await api.put(`/v1/traffic-director/links/${linkId}/rules/${rule.id}`, {
        name,
        destinationUrl,
        actionType,
