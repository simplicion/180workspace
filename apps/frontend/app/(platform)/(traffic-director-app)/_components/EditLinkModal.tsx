...
import { Settings, Sparkles, ArrowRightLeft, Eye, CheckCircle2, XCircle, Loader2 } from 'lucide-react';
...
export default function EditLinkModal({ isOpen, onClose, link, onSuccess }: EditLinkModalProps) {
  ...
  useEffect(() => {
    if (link) {
      ...
      setSafePageProxyMode(Boolean(link.safePageProxyMode));
  ...
  // Real-time debounced slug check on edit
  useEffect(() => {
    if (!link) return;
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await api.get('/api/v1/traffic-director/check-slug', {
          params: { slug: cleanSlug, excludeLinkId: link.id }
        });
        ...
        if (res.data?.data?.available) {
          ...
          setSlugMessage(res.data?.data?.reason || 'This slug is already in use.');
        }
      } catch (err: any) {
        setSlugStatus('unavailable');
      ...
      <div className="space-y-4 p-1">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            {slugStatus === 'available' && slug !== link?.slug && (
          ...
          <div className={`flex items-center rounded-xl border transition bg-gray-50 dark:bg-gray-800/50 overflow-hidden focus-within:ring-2 ${
            ...
            <div className="pr-3 flex items-center">
              ...
              {slugStatus === 'available' && slug !== link?.slug && <CheckCircle2 className="w-4 h-4 text-emerald-500" />}
