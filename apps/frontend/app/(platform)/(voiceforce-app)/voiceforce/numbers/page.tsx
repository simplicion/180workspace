...
export default function VoiceforceNumbersPage() {
  ...
  // Deletion Modal
  const [numberToDelete, setNumberToDelete] = useState<any | null>(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(`Number ${numberToDelete.e164Number || numberToDelete.number} released from carrier and disconnected.`);
      setNumberToDelete(null);
      fetchData();
    } catch (err: any) {
      toast.error(err.response?.data?.error || 'Failed to disconnect number');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      ...
      {/* Disconnect Line & No-Refund Advisory Modal */}
      <PlatformModal
        isOpen={Boolean(numberToDelete)}
        onClose={() => { if (!deleting) setNumberToDelete(null); }}
        title="Disconnect & Release Phone Number"
        icon={AlertTriangle}
        ...
        subHeader={
          <span className="text-xs text-rose-600 dark:text-rose-400 font-medium">
            Permanent telecom carrier release & no-refund notice
          </span>
        }
      >
        <div className="space-y-4">
          {/* Target Line Card */}
          <div className="p-3.5 rounded-xl bg-gray-50 dark:bg-gray-800/80 border border-gray-200/80 dark:border-gray-700/80 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center border border-indigo-100 dark:border-indigo-900/50 flex-shrink-0">
                <Smartphone className="w-5 h-5" />
              </div>
              <div>
                <h4 className="text-sm font-bold text-gray-900 dark:text-white tracking-wide">
                  {numberToDelete?.e164Number || numberToDelete?.number}
                </h4>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {numberToDelete?.friendlyName || (numberToDelete?.provider === 'verified_caller_id' ? 'Verified Business Caller ID' : 'Dedicated Virtual DID')}
                </p>
              </div>
            </div>
            <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 uppercase">
              {numberToDelete?.status || 'ACTIVE'}
            </span>
          </div>

          {/* No Refund Warning Box */}
          <div className="p-3.5 rounded-xl bg-rose-50/80 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 text-xs space-y-1">
          ...
          {/* Keep until End of Month Recommendation Box */}
          <div className="p-3.5 rounded-xl bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-200/80 dark:border-indigo-900/60 text-xs space-y-2">
            <div className="flex items-center gap-1.5 font-bold text-indigo-700 dark:text-indigo-300">
              <Calendar className="w-4 h-4 text-indigo-600 dark:text-indigo-400 flex-shrink-0" />
              <span>Recommendation: Keep Active Until End of Billing Period</span>
            </div>
            <p className="text-[11px] leading-relaxed text-indigo-900/90 dark:text-indigo-200/90">
              You have already paid for this line until{' '}
              <strong className="underline decoration-indigo-400 font-semibold">
                {numberToDelete?.nextRenewalDate 
                  ? new Date(numberToDelete.nextRenewalDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
                  : 'the end of your monthly cycle'}
              </strong>.
              If you plan to switch numbers or change lines, we strongly recommend keeping it active until your renewal date so your clients can still reach you without losing the month you already paid for.
            </p>
            <p className="text-[11px] text-gray-600 dark:text-gray-400 leading-relaxed pt-1 border-t border-indigo-100 dark:border-indigo-900/40">
              If you proceed with deletion now, this line is <strong>permanently released from the telecom carrier (Telnyx)</strong> immediately and cannot be reclaimed.
            </p>
          </div>

          {/* Action Buttons */}
          <div className="flex flex-col sm:flex-row items-center justify-end gap-2.5 pt-3 border-t border-gray-100 dark:border-gray-800">
            <button
              type="button"
              onClick={() => setNumberToDelete(null)}
              disabled={deleting}
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 text-xs font-semibold transition-colors cursor-pointer"
            >
              Keep Number (Recommended)
            </button>
            <button
              type="button"
              onClick={handleConfirmDelete}
              disabled={deleting}
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold shadow-md shadow-rose-600/20 transition-all disabled:opacity-50 flex items-center justify-center gap-1.5 cursor-pointer"
            >
              {deleting ? (
                <>
                  <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                  <span>Releasing from Telnyx...</span>
                </>
              ) : (
                <>
                  <Trash2 className="w-3.5 h-3.5" />
                  <span>Permanently Delete (No Refund)</span>
                </>
              )}
            </button>
          </div>
