...
export default function EditProjectModal({ project, onClose, onSuccess }: Props) {
    ...
    const [billingType, setBillingType] = useState(project.billingType || 'non_billable');
    ...
    const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set(project.memberIds || []));
    
    // Check if ownerId exists directly or inside an owner object
    const initialOwnerId = project.ownerId || (project.owner ? (project.owner.id || project.owner.id) : '');
