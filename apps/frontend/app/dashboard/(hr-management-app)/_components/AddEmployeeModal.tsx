...
const MODULE_GROUPS = navigation.filter(n => 'group' in n);
...
export default function AddEmployeeModal({ onClose, onSuccess, editUser }: Props) {
    ...
    const [collapsedGroups, setCollapsedGroups] = useState<string[]>(MODULE_GROUPS.map(g => g.group as string));
    ...
    const [form, setForm] = useState({
        ...
        password: '',
        employeeId: editUser?.employeeId || Math.floor(1000000000 + Math.random() * 9000000000).toString(),
        role: initialRole,
