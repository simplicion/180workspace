# Imara IMS & PitchIn 180: Comprehensive System Specification & Strategy

> [!IMPORTANT]
> **Name Selection:** Based on your request for an African common name for the management tool, we have selected **Imara IMS**. "Imara" is Swahili for *strong, firm, and resilient*—the perfect foundation for a startup's operational lifecycle.
...
2. **Imara IMS:** The enterprise-grade Intelligent Management System where startups actually run their business, track their growth, and manage their lifecycle.

By uniting these two platforms, a founder can join PitchIn 180 to find a co-founder, build their product using Imara IMS to manage their hired freelancers, track their financial projections, and seamlessly generate a VC-ready Pitch Deck directly from their live operational data to secure funding.
...
#### Marketing Imara IMS (Bottom of Funnel - Enterprise)
...
- **Acquisition Channels:**
  - **Internal Upsell:** When a startup on PitchIn 180 hits 5 team members, automatically prompt them to activate Imara IMS for payroll, HR, and project management.
  - **B2B Content Marketing:** Publish whitepapers on "Lifecycle Management for Startups" and "How to track projections vs actuals."
  - **VC Partnerships:** Partner with Venture Capital firms to offer Imara IMS to their portfolio companies to ensure standardized, transparent reporting.

### 2.2 Branding Guidelines
- **PitchIn 180 Tone:** Dynamic, community-driven, inspiring, fast-paced.
- **Imara IMS Tone:** Professional, secure, data-driven, reliable.
...
- **Data Ingestion:** The engine pulls live data from Imara IMS (hiring history, revenue projections, operational costs, active projects).
...
## 4. Imara IMS: The Startup Lifecycle Management Tool

Imara IMS is the backbone of the business. It is heavily modularized into the following systems:
...
- **The Lifecycle Budget:** Startups input their seed capital and projected burn rate. Imara IMS tracks this against actual payroll and operational expenses in real-time.
...
2. **Launch Stage:** Sarah and her CTO register their company on the platform. They activate Imara IMS.
3. **Growth Stage:** They hire 5 freelancers from the PitchIn community. Imara IMS tracks their tasks, hours, and payments.
...
3. **Execution:** John works inside Imara IMS. His tasks and hours are logged automatically.
...
- `apps/user-web`: The Next.js 15 frontend for the entire PitchIn 180 & Imara IMS ecosystem.
...
- **Imara IMS Roles:** `COMPANY_CEO`, `HR_MANAGER`, `PROJECT_MANAGER`, `EMPLOYEE`, `FREELANCER`
...
- Pushes live updates to the Imara IMS dashboard (e.g., when a task is moved to 'Done', all viewing users see the update instantly without polling).
...
- **Multi-Tenancy:** Imara IMS uses strict row-level security and tenant-ID filtering to ensure Company A can never access Company B's financial projections.
...
- **Caching Strategy:** Redis is deployed globally. All frequently accessed PitchIn 180 profiles and Imara IMS dashboard statistics are cached. Cache invalidation occurs via event-driven triggers in the Express backend.
...
By deploying **PitchIn 180** as the community gateway and **Imara IMS** as the operational engine, we are creating a closed-loop ecosystem. Startups never have to leave our platform. From the moment they have an idea, to finding a team, to managing operations, to raising capital via automated pitch decks—the entire journey lives here. 
