...
3. **Token Issuance:** A cryptographically signed **JSON Web Token (JWT)** is generated holding the User's ID, Company Tenant designation, and active access level.
...
The `tenant-db` middleware intercepts requests, extracts the JWT, and limits absolutely all Prisma database calls (Queries, Updates, Deletions) strictly to records mapping to `req.user.companyId`.
