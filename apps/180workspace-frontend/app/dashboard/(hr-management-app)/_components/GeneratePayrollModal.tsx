...
import { X, DollarSign, Calendar, User, Loader2, AlertCircle, Plus, XCircle, Info } from 'lucide-react';
...
export default function GeneratePayrollModal({ onClose, onSuccess }: Props) {
    ...
    useEffect(() => {
        if (form.employeeId && monthStr) {
            fetchPreview();
        }
    }, [form.employeeId, monthStr]);
    ...
    async function handleGenerate(e: React.FormEvent) {
        ...
        try {
            const payload = {
                ...
                // Include breakdown from preview
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-gray-900">Generate Professional Payroll</h2>
                            <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold italic">Automated Attendance Sync</p>
                        </div>
                    </div>
                    <button onClick={onClose} title="Close modal" className="w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center transition-colors">
                ...
                <form onSubmit={handleGenerate} className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
                    ...
                    <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-4 flex gap-4 transition-all hover:shadow-sm">
                        ...
                        <div className="flex-1">
                            ...
                            >
                                {generating ? <Loader2 className="w-3 h-3 animate-spin" /> : <DollarSign className="w-3.5 h-3.5" />}
                ...
                <div className="px-6 py-5 bg-gray-50/50 border-t border-gray-100 flex justify-end gap-3 rounded-b-2xl">
