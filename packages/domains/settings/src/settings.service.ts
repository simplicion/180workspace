...
export class SettingsService {
    ...
    static async testEmailConnection(data: any, user: any, dependencies: any) {
        ...
        const smtpHost = data.smtpHost || metadata.smtpHost || settings?.smtpHost;
        const smtpPort = data.smtpPort || metadata.smtpPort || settings?.smtpPort;
        const smtpUser = data.smtpUser || metadata.smtpUser || settings?.smtpUser;
        let smtpPass = data.smtpPass || metadata.smtpPass || settings?.smtpPass;
        if (smtpPass === '********') smtpPass = metadata.smtpPass || settings?.smtpPass;
        const smtpSecure = data.smtpSecure !== undefined ? data.smtpSecure : (metadata.smtpSecure !== undefined ? metadata.smtpSecure : settings?.smtpSecure);
        const emailFrom = data.smtpFrom || metadata.emailFrom || settings?.emailFrom || data.emailFrom || smtpUser;
