...
export class CrmCalculationService {
    /**
     * Calculates Recency, Frequency, and Monetary scores for accounts.
     * @param deals - Array of won deals with expectedCloseDate and value.
     * @returns - RFM metrics and segment.
     */
    static calculateRFM(deals: any[]) {
        if (!deals || deals.length === 0) return { r: 0, f: 0, m: 0, segment: 'None' };

        // Sort deals by date to get most recent
        const sortedDeals = [...deals].sort((a, b) => new Date(b.expectedCloseDate).getTime() - new Date(a.expectedCloseDate).getTime());

        const recency = moment().diff(moment(sortedDeals[0].expectedCloseDate), 'days');
        const frequency = sortedDeals.length;
        const monetary = sortedDeals.reduce((sum, d) => sum + (d.value || 0), 0);
     ...
     * @param opportunities - Array of opportunities with value and probability.
     * @returns - Expected and weighted sums.
     */
    static forecastPipeline(opportunities: any[]) {
        let expected = 0;
        let weighted = 0;
     ...
     */
    static scoreLead(lead: any, config?: any) {
     ...
     */
    static calculateWinProbability(stage: string, engagementScore: number, config?: any) {
        const stages = config || {
     ...
     */
    static calculateCustomerRiskIndex(lostDealsCount: number, staleContactsCount: number) {
     ...
     */
    static calculateRepProductivity(dealsClosed: number, revenueValue: number, activitiesCount: number, config?: any) {
        const weights = config || {
