...
async function runFormBuilderTests() {
  ...
  try {
    ...
    const nameField = testForm.fields.find(f => f.label === 'Full Name');
    const emailField = testForm.fields.find(f => f.label === 'Work Email');
    const budgetField = testForm.fields.find(f => f.label === 'Monthly Ad Budget ($)');
    const fileField = testForm.fields.find(f => f.label === 'Brand Guidelines / Deck');
    const ratingField = testForm.fields.find(f => f.label === 'Service Level Rating');
    const selectField = testForm.fields.find(f => f.label === 'Business Vertical');
    ...
    const targetFormInList = formsList.find(f => f.id === testForm.id);
