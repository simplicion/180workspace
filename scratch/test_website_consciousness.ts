import { websiteAIBuilderService, formAIBuilderService, documentAIBuilderService } from '@workspace/ai';
...
async function runTests() {
    ...
    const turn1Result = await websiteAIBuilderService.patchAST(
        'mock-web-1',
        'landing page for our product for a medicine product',
        {
            prompt: 'landing page for our product for a medicine product',
    ...
    const turn2Valid = 
        ...
        (turn2Result.reply?.toLowerCase().includes('confirmed') || turn2Result.reply?.toLowerCase().includes('added') || turn2Result.reply?.toLowerCase().includes('synthesized'));
    ...
    const formTurn1Result = await formAIBuilderService.patchAST(
        'mock-form-1',
        'form for a medicine product',
        {
            prompt: 'form for a medicine product',
            companyId: 'mock-company',
            userId: 'mock-user',
    ...
    const docTurn1Result = await documentAIBuilderService.patchAST(
        'mock-doc-1',
        'create an agreement for pharmaceutical supply',
        {
            prompt: 'create an agreement for pharmaceutical supply',
            companyId: 'mock-company',
            userId: 'mock-user',
